...
# 3. CLI 인수 파싱
# ========================================
parser = argparse.ArgumentParser(description="유튜브 인스트림 광고 분류 Agent")
...
class VideoInstreamClassification(BaseModel):
    ...
    instream_type: Literal["건너뛰기가능", "건너뛰기불가", "숏츠광고", "범퍼광고", "None"] = Field(
        description=(
            ...
            "숏폼 형태의 광고 유형이라면 숏츠광고로 분류한다.(입력 데이터 내 url의 형태가 'http://youtube.com/shorts/{video_id}' 형태면 숏폼)"
...
# ── Tool 2: 자막 수집 ────────────────────────────────────────
def get_video_transcript(video_id: str, max_chars: int = 1500) -> dict:
    """
...
def analyze_thumbnail(thumbnail_url: str) -> dict:
    ...
    # 이미지 다운로드
    try:
        img_bytes = httpx.get(thumbnail_url, timeout=10, follow_redirects=True).content
    except Exception as e:
        return {**empty, "error": f"이미지 다운로드 실패: {e}"}

    # Gemini Vision 호출 (v1beta1 엔드포인트)
    try:
        client = genai.Client(
            vertexai=True,
            project=GCP_PROJECT,
...
async def run_agent(df: pd.DataFrame) -> tuple[list, dict, dict]:
    ...
    agent = Agent(
        ...
        retries=3,
        # ── Tool 등록 ─────────────────────────────────────
        # Tool 1: CSV 로컬 데이터에서 메타데이터 조회 (API 호출 없음)
        # Tool 2: youtube-transcript-api로 자막 수집
        # Tool 3: Gemini Vision으로 썸네일 분석
...
async def main():
    ...
    VIDEO_METADATA_STORE = {
        row["video_id"]: {
            ...
            "has_paid_product_placement": row.get("has_paid_product_placement", False),
            "view_count"                : row.get("view_count", 0),
            "like_count"                : row.get("like_count", 0),
            "thumbnail_url"             : row.get("thumbnail", ""),
            "upload_date"               : row.get("upload_date", ""),
            "verdict"                   : row.get("verdict", ""),   # shorts/longform
