...
def build_guideline_report(ch, ana, bench, domain_label, accent, top5_data=None):
    ...
    if stats:
        ...
        analysis_block = (
            ...
            f'GCP Project ID 입력 시 AI 기반 이미지 분석 통계가 여기에 표시됩니다.</div>'
...
def yt_find_channel(q, key):
    if q.strip().startswith("UC"):
        return q.strip()
    r = requests.get("https://www.googleapis.com/youtube/v3/search",
        params={"part":"snippet","type":"channel","q":q,"maxResults":1,"key":key}, timeout=10)
    d = r.json()
    if "error" in d: raise Exception(d["error"]["message"])
    if not d.get("items"): raise Exception("채널을 찾을 수 없습니다")
...
{{
  ...
  "prompt_ko": "한국어 1문장 요약"
    ...
    mdl = GenerativeModel("gemini-2.5-flash")
  ...
  "improvements": [
    ...
    if img_url:
        try:
            ...
            contents.append(Part.from_data(data=r.content, mime_type="image/jpeg"))
        except Exception:
            pass
...
def gemini_gen_image(
    ...
    img_model = ImageGenerationModel.from_pretrained(model)
    ...
    raw_bytes = result.images[0]._image_bytes
...
Be specific and concise. English only. Max 200 words."""
    ...
    mdl_desc = GenerativeModel("gemini-2.5-flash")
    ...
    img_model = ImageGenerationModel.from_pretrained("imagen-4.0-generate-001")
    result = img_model.generate_images(
        prompt=final_prompt,
        number_of_images=1,
        aspect_ratio="16:9",
    ...
    if not result.images:
        raise Exception("이미지 생성 실패 (safety filter 또는 프롬프트 정책 문제)")
...
def render_channel_result(prefix, accent, bench, domain_label):
    ...
    with hc1:
        if ch.get("avatar"): st.image(ch["avatar"], width=64)
    with hc2:
        st.markdown(
    ...
    if sel:
        ...
        with ap2:
            ...
            if cached and cached.get("video_id") == sel["id"]:
                ...
                if not st.session_state.get("vertex_project",""):
                    st.markdown('<div class="api-notice">💡 사이드바에서 GCP Project ID를 입력하면 썸네일 AI 분석을 받을 수 있습니다.</div>', unsafe_allow_html=True)
                else:
    ...
    with st.expander("📋 맞춤형 썸네일 전략 가이드라인 보고서", expanded=True):
        ...
        if _proj:
            ...
            top5_data = {}
            st.markdown(
                '<div class="api-notice">💡 사이드바에서 GCP Project ID를 입력하면 '
                '썸네일 AI 분석 기반 보고서를 생성할 수 있습니다.</div>',
                unsafe_allow_html=True)
...
with st.sidebar:
    ...
    yt_key     = st.text_input("YouTube Data API v3",    type="password", key="yt_key",     placeholder="AIza...")
    vertex_project  = st.text_input("GCP Project ID", key="vertex_project",  placeholder="my-gcp-project")
    ...
    Vertex AI 기능:<br>
    · 프롬프트 자동 생성 (gemini-2.5-flash)<br>
...
if page == "썸네일 분석":
    ...
    with tab_fnb:
        ...
        if fnb_go:
            if not yt_key:
                ...
                with st.spinner("채널 데이터 수집 중 (롱폼/숏폼 구분 포함)..."):
                    try:
                        cid  = yt_find_channel(fnb_q, yt_key)
                        ch   = yt_channel_info(cid, yt_key)
                        vids = yt_fetch_videos(cid, yt_key, max_results=20)
                        # redirect 기반 롱폼/숏폼 분류
                        vid_ids  = [v["id"] for v in vids]
                        verdicts = classify_videos_redirect(vid_ids, workers=min(len(vid_ids), 20))
                        for v in vids:
                            v["verdict"]      = verdicts.get(v["id"], "unknown")
                            v["is_longform"]  = (v["verdict"] == "longform")
                            v["is_shortform"] = (v["verdict"] == "shorts")
                        ana  = analyze_channel(vids, FNB, "FnB")
    ...
    with tab_it:
        ...
        if it_go:
            if not yt_key:
                ...
                with st.spinner("채널 데이터 수집 중 (롱폼/숏폼 구분 포함)..."):
                    try:
                        cid  = yt_find_channel(it_q, yt_key)
                        ch   = yt_channel_info(cid, yt_key)
                        vids = yt_fetch_videos(cid, yt_key, max_results=20)
                        # redirect 기반 롱폼/숏폼 분류
                        vid_ids  = [v["id"] for v in vids]
                        ...
                        for v in vids:
                            ...
                            v["is_longform"]  = (v["verdict"] == "longform")
                            v["is_shortform"] = (v["verdict"] == "shorts")
                        ana  = analyze_channel(vids, IT, "IT")
    ...
    with tab_gen:
        ...
        if not st.session_state.get("vertex_project",""):
            st.markdown('<div class="api-notice">💡 사이드바에서 <strong>GCP Project ID</strong>를 입력하세요. Vertex AI API가 활성화된 GCP 프로젝트가 필요합니다.</div>', unsafe_allow_html=True)
        ...
        with sub_improve:
            ...
            if _ref_thumb_url or _ref_thumb_img:
                ...
                if imp_auto:
                    ...
                    # 분석 결과에서 현황 + 개선 힌트 추출
                    _elems_a  = (_cur_ana or {}).get("elements", {})
                    _imprv_a  = (_cur_ana or {}).get("improvements", [])
                    _strgs_a  = (_cur_ana or {}).get("strengths", [])

                    _current_a = ", ".join(filter(None, [
                        _elems_a.get("main_objects",""),
                        _elems_a.get("color_palette",""),
                        _elems_a.get("person_details",""),
                    ]))[:200]

                    _hints_a = []
                    for tip in _imprv_a[:3]:
                        if isinstance(tip, dict) and tip.get("prompt_hint"):
                            _hints_a.append(tip["prompt_hint"])

                    _keep_a = _strgs_a[0] if _strgs_a else ""

                    # 사용자 수정 지시 + 분석 힌트 합치기
                    _user_kw = imp_keywords.strip()
                    _combined = (
                        (_user_kw + ". " if _user_kw else "") +
                        ("; ".join(_hints_a) if _hints_a else "improve overall quality")
                    )
                    ...
                    _new_prompt = (
                        f"Improved YouTube thumbnail for {_domain_ctx_a}. "
                        + (f"Original: {_current_a}. " if _current_a else "")
                        + (f"Keep: {_keep_a}. " if _keep_a else "")
                        + f"Changes: {_combined}. "
