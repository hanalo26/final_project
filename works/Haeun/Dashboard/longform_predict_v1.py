...
# ── 연동 가이드 ──
#
#   이 페이지의 [예측 결과 저장] 버튼을 누르면
#   Dashboard/saved/longform_ml_result.md 파일이 생성(또는 덮어쓰기)됩니다.
#
#   가이드라인 페이지 ("롱폼 agent 결과 반영" 탭)에서 아래 코드로 읽어주세요.
#   ※ 가이드라인 파일 위치에 따라 경로가 달라집니다.
#
#   [Case 1] 가이드라인 파일이 Dashboard/pages/guideline.py 인 경우
#     result_path = Path(__file__).parent.parent / "saved" / "longform_ml_result.md"
#
#   [Case 2] 가이드라인 파일이 Dashboard/guideline.py 인 경우
#     result_path = Path(__file__).parent / "saved" / "longform_ml_result.md"
#
#   공통 코드:
...
SAVED_DIR.mkdir(exist_ok=True)   # saved/ 폴더 없으면 자동 생성
RESULT_PATH     = SAVED_DIR / "longform_ml_result.md"
...
def get_ai_comment(prompt: str) -> str:
    result = asyncio.run(get_comment_agent().run(prompt))
...
# load_models()에서 반환되는 6개의 값을 바로 언패킹해서 전역변수로 사용
perf_model, perf_meta, perf_best_name, comment_model, comment_meta, comment_best_name = load_models()
...
# ═══════════════════════════════════════════════════════════════════════════
def page_simulator():
    st.title("📊 YouTube 영상 성과 및 댓글반응 예측")
    st.caption(
        "영상 정보를 입력하면 AI 모델이 성공 확률과 댓글 내 긍정 반응 비율을 예측합니다. "
        "예측에 영향을 준 주요 요인 TOP 5와 AI 전략 코멘트도 함께 제공합니다."
    )
    st.divider()
    ...
    with ti_c1:
        # st.markdown(
        #     '<label style="font-size:0.875rem;font-weight:400;color:inherit;">'
        #     '태그 수</label>',
        #     unsafe_allow_html=True,
        # )
    ...
    with ti_c2:
        ...
        with len_c1:
            video_min = st.number_input("", min_value=0, max_value=300, value=5, step=1, label_visibility="collapsed")
        with len_c2:
            st.markdown("<div style='padding-top:-5px;font-size:0.9rem;'>분</div>", unsafe_allow_html=True)
        with len_c3:
            video_sec_extra = st.number_input("", min_value=0, max_value=59, value=0, step=1, label_visibility="collapsed")
        with len_c4:
    ...
    with sel_c2:
        category_name = st.selectbox("유튜브 카테고리", CATEGORY_NAME_OPTIONS)
    ...
    with tog_c1:
        caption_use = st.toggle("자막 포함 여부", value=True)
    with tog_c2:
        embeddable = st.toggle("외부 임베드 허용 여부", value=True)
    with tog_c3:
    ...
    shap_top5 = []   # [(label, abs_val, signed_val), ...] ─ 저장 섹션에서도 사용
    ...
    try:
        ...
        sv        = explainer(X_transformed).values[0]   # shape: (n_features,)
        ...
        emb_signed = emb_abs_sum * np.sign(emb_sign_sum)   # 부호 붙인 크기
        ...
        st.info(f"SHAP 분석을 수행할 수 없습니다: {e}")
...
"""
    ...
    # ═══════════════════════════════════════════════════════
    st.divider()
    st.markdown('<div class="sec-label">예측 결과 저장</div>', unsafe_allow_html=True)
    st.caption("저장 버튼을 누르면 가이드라인 페이지의 '롱폼 agent 결과 반영' 탭에 자동으로 반영됩니다.")
    st.caption(f"저장 위치: `saved/longform_ml_result.md`")
    ...
    if st.button("💾 가이드라인 탭에 저장", use_container_width=True):
        try:
            RESULT_PATH.write_text(md_content, encoding="utf-8")
            st.success(f"저장 완료: `saved/longform_ml_result.md`")
        except Exception as e:
            st.error(f"저장 실패: {e}")
