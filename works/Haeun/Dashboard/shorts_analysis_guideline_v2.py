...
def render_video_card(row):
    ...
    st.markdown(f"""
    <div class="yt-card">
        <div style="font-size:13px;font-weight:600;color:#111827;margin-bottom:5px">{title}</div>
        <div class="yt-muted" style="margin-bottom:6px">채널: {channel}</div>
        ...
        <div class="yt-muted" style="margin-bottom:6px">{reason}</div>
        <a href="{url}" target="_blank" style="color:#2563eb;font-size:12px">영상 열기 →</a>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown(
    ...
    '<span style="font-size:16px;font-weight:700">Short<span style="color:#ff0000">Strategy</span></span>' +
    '<span style="font-size:12px;color:#6b7280;margin-left:4px">| 페르소나 기업 맞춤형 숏츠 진단</span>' +
    '</div>',
    unsafe_allow_html=True,
)
    ...
    if run_button:
        ...
        try:
            with st.status("숏츠 수집 및 영상 분석 중...", expanded=True) as status:
                ...
                shorts_df = collect_recent_shorts(channel_url, limit=int(limit))
...
def _fmt(v):
    return f"{v:.2f}" if v is not None and str(v) not in ("nan", "None") else "-"
...
m2.metric("첫 3초 전략",   persona_summary.get("main_first_3sec",    "-"))
...
if not tasks:
    ...
    for col, task in zip(imp_cols, tasks):
        with col:
            st.markdown(
                f'<div class="yt-card" style="border-left:3px solid {accent_color};height:100%">'
                f'<div style="font-size:10px;font-weight:700;color:{accent_color};margin-bottom:4px">⚠️ 개선 필요</div>'
                ...
                f'<div style="font-size:11px;background:#f3f4f6;border-radius:6px;padding:8px 10px;'
                f'color:#1e40af;line-height:1.6">💡 {task["action"]}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
...
st.markdown(
    ...
    f'첫 3초 전략: <b>{persona_summary.get("main_first_3sec", "-")}</b><br>'
    ...
    f'<b>첫 3초:</b> {guideline.get("첫 3초 전략", "-")}'
    f'</div></div>'
    f'</div>'
...
with gl_left:
    for label, key in [
        ...
        st.markdown(
            ...
            f'<div style="font-size:12px;font-weight:700;color:#111827;margin-bottom:4px">{label}</div>'
            f'<div style="font-size:12px;color:#374151;line-height:1.7">{guideline[key]}</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
with gl_right:
    st.markdown('<div class="yt-card">', unsafe_allow_html=True)
    st.markdown(
        '<div style="font-size:12px;font-weight:700;color:#111827;margin-bottom:8px">✅ 제작 전 체크리스트</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        f'<div class="yt-card">'
        f'<div style="font-size:12px;font-weight:700;color:#111827;margin-bottom:8px">📌 다음 숏츠 기획 아이디어</div>'
        f'{ideas_html}</div>',
        unsafe_allow_html=True,
    )
...
- 첫 3초 전략: {persona_summary.get("main_first_3sec", "-")}
...
주의:
- 최근 {persona_summary.get("n", 5)}개 숏츠 기반 빠른 진단임을 명시할 것
...
with btn_col:
    gen_btn = st.button("🤖 AI 종합 평가 생성", key="gen_ai_btn", use_container_width=True)
...
if st.session_state["ai_eval_result"]:
    st.markdown('<div class="yt-card" style="margin-top:10px">', unsafe_allow_html=True)
