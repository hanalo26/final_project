...
#   Dashboard/saved/longform_{도메인}_{제목}_{일시}.md 파일이 생성(또는 덮어쓰기)됩니다.
#
#   가이드라인 페이지 ("롱폼 agent 결과 반영" 탭)에서 아래 코드로 읽어주세요.
#   ※ 가이드라인 파일 위치에 따라 경로가 달라집니다.
#
#   [Case 1] 가이드라인 파일이 Dashboard/pages/guideline.py 인 경우
#     result_path = Path(__file__).parent.parent / "saved" / "longform_{도메인}_{제목}_{일시}.md"
#
#   [Case 2] 가이드라인 파일이 Dashboard/guideline.py 인 경우
#     result_path = Path(__file__).parent / "saved" / "longform_{도메인}_{제목}_{일시}.md"
#
#   공통 코드:
...
def get_ai_comment(prompt: str) -> str:
    """별도 스레드에서 새 이벤트 루프를 생성해 실행.
    에이전트는 메인 스레드(Streamlit 컨텍스트)에서 미리 생성 후 스레드에 전달.
    """
    # 에이전트를 메인 스레드에서 먼저 생성 (st.secrets 접근 가능한 컨텍스트)
...
def _render_results(r: dict):
    ...
    st.caption("저장 위치: `saved/longform_{도메인}_{영상제목 일부}_{분석일시}.md`")
    ...
    if st.button("💾 가이드라인 탭에 저장", use_container_width=True):
        try:
            # 파일명: longform_{도메인}_{제목 앞 10자}_{일시}.md
            # 제목에서 파일명에 쓸 수 없는 문자 제거
            title_slug = re.sub(r'[\\/*?:"<>|\s]', '', title_input)[:10]
            timestamp  = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
            filename   = f"longform_{dom_key}_{title_slug}_{timestamp}.md"
            save_path  = SAVED_DIR / filename
            save_path.write_text(md_content, encoding="utf-8")
            st.success(f"저장 완료: `saved/{filename}`")
