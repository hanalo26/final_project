...
- 모든 호출: google-genai SDK (Vertex AI mode)
- 모델 ID는 secrets.toml에서 주입:
    GEMINI_IMAGE_MODEL (이미지 생성), GEMINI_VISION_MODEL (분석), GEMINI_PROMPT_MODEL (텍스트/번역)
...
# Vertex AI
# Legacy vertexai SDK는 더 이상 사용하지 않음 (google-genai SDK로 통일).
# 단, 다른 import 호환을 위해 placeholder 유지 — 실제 호출엔 안 쓰임.
class _DeprecatedShim:
    def __init__(self, name): self._name = name
    def __call__(self, *a, **k):
        raise RuntimeError(
            f"{self._name} (legacy vertexai SDK) is no longer used. "
            "All calls go through _get_genai_client() (google-genai SDK)."
        )

GenerativeModel = _DeprecatedShim("GenerativeModel")
Part = _DeprecatedShim("Part")
GenerationConfig = _DeprecatedShim("GenerationConfig")
SafetySetting = _DeprecatedShim("SafetySetting")
HarmCategory = _DeprecatedShim("HarmCategory")
HarmBlockThreshold = _DeprecatedShim("HarmBlockThreshold")
try:
...
def _get_gcp_credentials():
    """
    로컬: gcloud ADC (gcloud auth application-default login)
    클라우드: st.secrets["gcp_service_account"]의 Service Account JSON
    둘 다 자동 지원.
    """
...
GEMINI_IMAGE_MODEL = st.secrets.get(
    "GEMINI_IMAGE_MODEL",
...
_genai_client_cache: dict = {}   # {(project_id, location): genai.Client}


def _get_genai_client(project_id: str, location: str = "us-central1"):
    """
    공용 Vertex AI google-genai client (SA 인증 + 캐시).
    - 모든 텍스트/이미지/Vision 호출이 동일 SDK 사용
    - location='global'도 지원 (gemini-3 계열 image gen용)
    """
    from google import genai

    if location != "global" and location not in _SUPPORTED_LOCATIONS:
        location = "us-central1"

    cache_key = (project_id, location)
    if cache_key not in _genai_client_cache:
        creds = _get_gcp_credentials()
        _genai_client_cache[cache_key] = genai.Client(
            vertexai=True,
            project=project_id,
            location=location,
            credentials=creds,
        )
    return _genai_client_cache[cache_key]


def init_vertex(project_id: str, location: str = "us-central1"):
    """
    호환성 유지용 — 더 이상 vertexai.init 호출하지 않음.
    실제 클라이언트는 각 함수에서 _get_genai_client()로 받음.
    호출자는 그대로 두면 됨 (no-op).
    """
    _vertex_initialized[f"{project_id}:{location}"] = True
...
def gemini_image_generate_with_reference(
    prompt: str,
    ...
    """
    Vertex AI + google-genai SDK로 참고 이미지 기반 새 이미지 생성.

    - gemini-3 계열 (Pro/Flash Preview): location 강제 'global' (Pro는 global endpoint only)
    - gemini-2.5-flash-image: us-central1 등 regional endpoint OK
    - 인증: Service Account (secrets.toml의 [gcp_service_account]) 또는 ADC 자동 폴백

    입력: 텍스트 프롬프트 + 참고 이미지 bytes
    반환: PNG bytes
    """
    from google import genai
    from google.genai import types

    effective_location = "global" if "gemini-3" in model else location
    creds = _get_gcp_credentials()

    print(f"[gemini_image_generate_with_reference] model={model} location={effective_location} (input location={location})", flush=True)
    ...
    image_part = types.Part.from_bytes(data=ref_image_bytes, mime_type=ref_mime_type)
    ...
    raise Exception("Vertex AI(google-genai) 응답에서 이미지 데이터를 찾을 수 없습니다")

# ──────────────────────────────────────────────
# 1. 텍스트 생성 (gemini-2.5-flash-lite on Vertex AI)
# ──────────────────────────────────────────────
def gemini_generate_text(
    ...
    resp = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            max_output_tokens=IMAGE_GEN_CONFIG["vision_max_tokens"],
        ),
    )
    return (resp.text or "").strip()

# ──────────────────────────────────────────────
# 2. 비전 분석 (GEMINI_VISION_MODEL on Vertex AI)
# ──────────────────────────────────────────────
def gemini_analyze_image(
    ...
    if is_b64:
        ...
        r = requests.get(img_url_or_b64, timeout=10)
        img_bytes = r.content
    ...
    resp = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            temperature=IMAGE_GEN_CONFIG["vision_temperature"],
            max_output_tokens=IMAGE_GEN_CONFIG["vision_max_tokens"],
        ),
    )
    return (resp.text or "").strip()
...
def gemini_gen_prompt(
    ...
    # ── 키워드에서 구체적 시각 요소 추출 (한국 유튜브 트렌드 + 고유명사 대응) ──
    keyword_analysis_prompt = f"""You are a visual translator for Korean YouTube thumbnails.
Convert these Korean keywords into concrete visual elements an image model can render: "{keywords}"

Rules:
- Think like a Korean YouTube viewer: what is THE single most clickable visual?
- Identify any concrete subject types: person(s), food, product, character/IP, brand mark, setting.
- For brand collabs (e.g. "스폰지밥 CU 콜라보"): describe both — character's exact look
  (SpongeBob = yellow square sponge, big eyes, white shirt, red tie) AND brand color/identity
  (CU = blue/lime green convenience store).
- For foods (e.g. "크림빵"): describe the food in detail — golden-brown crust, visible white
  cream filling, glossy texture, steam if hot, condensation if cold.
- For products: describe color, shape, packaging label, material finish.
- For people: describe pose, expression, framing — but DO NOT specify face placement
  (the final prompt will keep faces clear of any text/logo overlay).
- For settings: 1-line concrete background (kitchen / convenience store / studio / street / desk).
- Keep each field concise (under 30 words). All English.

Return ONLY a JSON:
{{
  "visual_subjects": "main visual subjects with appearance details — what fills the frame",
  ...
  "scene_context": "background or scene that fits the keyword context"
    ...
    from google.genai import types
    _client = _get_genai_client(project_id, location)
    visual_elements = {"visual_subjects": keywords, "key_objects": "", "scene_context": ""}
    try:
        _kw_resp = _client.models.generate_content(
            model=GEMINI_PROMPT_MODEL,
            contents=[keyword_analysis_prompt],
            config=types.GenerateContentConfig(
                temperature=0.3,
        ...
        _kw_data = _json.loads((_kw_resp.text or "").strip())
        visual_elements.update(_kw_data)
    except Exception:
        pass  # fallback: 원본 키워드 그대로 사용

    system_prompt = f"""You are an expert Korean YouTube thumbnail prompt engineer.
Your output drives {GEMINI_IMAGE_MODEL} to generate a thumbnail that will be tested for click-through performance.

Write a 150-200 word English prompt that describes every visual element of the thumbnail with precision.

[Channel & Content Context — USE these as the basis for text/logo decisions below]
{ch_ctx}
Domain: {domain} | Category: {category} | Color tone: {color_tone}
Original Keywords (the thumbnail's concept/title): {keywords}
* The keywords above ARE the content concept. Treat them as the de-facto thumbnail title
  and derive the headline text, sub-tag, and logo choices DIRECTLY from them. *
...
6. Wide horizontal framing (16:9 landscape ratio)
7. Background must be simple and NOT compete with the main subject
8. Every named object/character/food MUST be visually recognizable and detailed
9. Reserve roughly 30% of the frame (one side) as a relatively clean area where text can sit legibly

[TEXT & LOGO STRATEGY — Korean YouTube best practice]
Korean YouTube thumbnails earn clicks through a clear visual hook + 1 short punchy text line.
Your job is to DECIDE — based on the keywords, channel name, and category above — what text and logos
the thumbnail should contain. This is NOT optional: derive concrete choices and write them into the prompt.

Step 1) HEADLINE TEXT — derive from the keywords above:
- Read "Original Keywords" and extract the single strongest hook word/phrase a viewer would click for.
  · Numbers (e.g. "1위", "단돈 3천원", "30초")
  · Surprise/contrast words ("실화", "충격", "공개", "최초", "역대급")
  · Brand collab names ("스폰지밥 X CU")
  · Action verbs / outcomes ("성공", "실패", "변신")
- Translate this hook into 2-5 Korean characters MAX, then write it in the prompt EXACTLY in this pattern
  (replace <HOOK_WORD> with the actual Korean characters you chose, in quotes):
  "Bold sans-serif Korean text \"<HOOK_WORD>\" in heavy weight, white fill with thick black outline (8-10px)
   and strong drop shadow, occupying about 25-35% of the frame width, placed on the empty side
   (left or right) of the composition, baseline aligned with the subject's eye level."
- If no hook word emerges naturally (pure mood/visual content), explicitly say "No headline text"
  and leave the clean side empty.

Step 2) SUB-TAG (optional) — 1-3 Korean characters, only if it adds clear information
  (e.g. "EP.3", "신상", "후기"). Place below the headline, half its size.

Step 3) LOGO / BRAND MARK — derive from the keywords, channel name, or category:
- If keywords mention a brand/product (e.g. "CU", "SKT", "삼성") OR the channel context indicates a brand:
  describe a small clean brand logo or wordmark in the lower-left OR lower-right corner,
  approximately 8-12% of the frame width. Use the actual brand's primary colors.
- If the category is "브랜드 이미지형" or "제품 홍보형": logo presence is REQUIRED.
- Otherwise the logo is optional — omit if the keywords don't justify one.

Step 4) CATEGORY-DRIVEN TEXT TONE:
- 예능/콘텐츠형: punchy emotional words ("대박", "충격", "실화")
- 정보 전달형: factual/numeric headline ("3분 요약", "TOP 5")
- 인터뷰/인물형: name + role or quote-style ("인터뷰", short keyword)
- 브랜드 이미지형: brand slogan-style minimal text
- 리뷰/비교형: comparison or verdict word ("비교", "최고", "vs")
- 제품 홍보형: product name + 1 benefit word

Step 5) HARD RULES:
- Render ONLY text justified by the keywords/channel/category. NEVER invent unrelated words.
- Maximum total visible text: 1 headline + 1 sub-tag + 1 logo. No paragraphs.
- Korean text MUST stay readable on a 320px-wide mobile thumbnail.
- If you write text in the prompt, name the exact Korean characters in quotes so the image
  model knows what to render.

Step 6) FACE PROTECTION (NON-NEGOTIABLE):
- Text and logos MUST NEVER overlap or cover any person's face. Eyes, eyebrows, nose, mouth,
  and chin must remain fully visible and unobstructed.
- Place the headline and sub-tag on the CLEAN side opposite the subject's face — left if the
  face is on the right, right if the face is on the left.
- Logo goes in the lower-left or lower-right corner only, never near the face region.
- If the subject's face fills the center, push the text to the bottom strip below the chin
  (max 20% of frame height) instead of overlaying.
- If avoiding face overlap is impossible at the requested text size, REDUCE text size before
  ever crossing the face.

[TYPOGRAPHY & VISUAL POLISH — design quality]
- Korean font: bold heavy sans-serif (Noto Sans KR Bold / Pretendard ExtraBold style).
  Avoid thin serifs, scripts, or decorative fonts. Tight letter spacing (≈ -0.02em) for impact.
- Text color contrast rule (decide based on text-region background):
  · Light/bright bg → solid black or deep brand color + 6-8px white halo
  · Dark bg        → solid white + 8-10px black outline + soft drop shadow
  · Highly saturated bg → white fill + thick black outline + subtle colored halo
- Text container choice (pick ONE that fits the layout cleanly):
  · plain overlay with outline + shadow (default, cleanest)
  · solid rectangular badge / angled banner (only when it adds editorial polish)
  Never random shapes or gradients on text.
- Logo treatment: actual brand primary-color logo if identifiable; otherwise single-color
  wordmark/monogram. Logo NEVER overlaps the subject's face or main product.
- Visual hierarchy (size + contrast must follow this order):
  subject (face/product) > headline text > sub-tag > logo > background.

[ANALYTICS-DRIVEN CRITERIA — must be measurable for CTR analysis]
- Mobile-first sizing: viewed at 320-360px wide. Headline character height ≥ 12% of frame height.
- Subject coverage: face/main product occupies 50-70% of frame (close-up tier).
- Brightness: high overall luminance — keep deep shadow areas < 40% of frame.
- Saturation: vivid palette so the Vision analyzer classifies it as "vivid/saturated", not "muted".
- Domain consistency: text and logo colors do NOT clash with the {domain} palette defined in
  [Domain Visual Style] and [Color Guidance].
- Category fit: the [Category-Specific MANDATORY Requirements] above are the proven success
  patterns for this domain — explicitly honor them (person count, expression, framing).
...
- Do NOT fill the frame with text walls; thumbnails compete with images, not paragraphs

[DECISION PROCESS — do this BEFORE writing prompt_en]
Step A) From the keywords, decide:
  - Will there be a HEADLINE text? (yes/no + chosen Korean 2-5 chars in quotes)
  - Will there be a SUB-TAG? (yes/no + chosen 1-3 chars)
  - Will there be a LOGO? (yes/no + brand name)
Step B) Decide composition:
  - Where does the subject's face/main product sit (left / center / right)?
  - Which side is CLEAN for text placement (left bottom / right bottom / bottom strip)?
Step C) Cross-check Face Protection: confirm the chosen text/logo positions do NOT overlap the face.
  If they do, reposition before writing prompt_en.

Respond ONLY in this JSON format (no markdown fences, valid JSON only):
{{
  "prompt_en": "detailed English prompt for the image model, 150-200 words, describing every visual element specifically: subject + pose/expression + background + color palette + headline text (with the exact Korean characters in quotes, font/outline/position) + sub-tag + logo (placement) + face protection note. Must be a single block of prose.",
  "prompt_ko": "한국어 구조화 가이드. 다음 섹션을 그대로 포함하고 각 섹션 아래 항목을 채워 쓰세요. 사용자가 textbox에서 읽고 수정할 수 있도록 자연스러운 줄바꿈을 넣으세요:\\n\\n[목표]\\n• 한 줄로 이 썸네일이 무엇을 어필하는지\\n\\n[주요 피사체]\\n• 인물/제품/캐릭터/음식 (한국어)\\n\\n[배경·분위기]\\n• 1-2줄\\n\\n[색감·조명]\\n• 도메인+색조 결합\\n\\n[헤드라인 텍스트]\\n• 후크 단어 (정확한 한국어 글자) 또는 '없음'\\n\\n[서브태그/로고]\\n• 있다면 한국어로 명시, 없으면 '없음'\\n\\n[얼굴 보호]\\n• 텍스트/로고가 인물 얼굴을 가리지 않는 배치 명시\\n\\n[제약]\\n• 16:9, 모바일 가독성, 채도/밝기 기준"
}}"""
...
def gemini_gen_improvement_prompt(
    ...
    """기존 썸네일 분석 결과 + 사용자 수정 지시를 바탕으로 이미지 생성용 개선 프롬프트 생성"""
    from google.genai import types
    client = _get_genai_client(project_id, location)
    ...
    prompt = f"""You are an expert Korean YouTube thumbnail prompt engineer.
Generate one improved English image-gen prompt based on the analysis below.
The result must look like a REAL Korean YouTube thumbnail — bold, punchy, eye-catching, and designed
for click-through rate analysis (the output will be re-scored by a Vision analyzer).

[Domain]
{domain_ctx}

[Current thumbnail analysis (from Vision)]
...
- Subject (face/product) fills 50-70% of frame — close-up tier
- HIGH CONTRAST between subject and background, subject must POP
- Wide horizontal 16:9 framing, mobile-readable at 320-360px

[TEXT & LOGO RULES — CRITICAL, derive from current analysis above]
- PRESERVE every text overlay and logo present in the original thumbnail. NEVER remove them.
- You MAY improve their styling only: font, weight, size, position, color contrast,
  outline, drop shadow, integration with background.
- Keep the original wording and brand identifiability intact.
- If the analysis above mentions an existing brand mark, keep its placement (or move to a
  cleaner corner, lower-left or lower-right, 8-12% frame width).
- FACE PROTECTION (NON-NEGOTIABLE): text and logos MUST NEVER cover any person's face.
  Eyes, eyebrows, nose, mouth, and chin must stay fully visible. If the original placement
  overlapped the face, MOVE it to a clean side or below the chin while improving styling.
  Shrink text before ever crossing the face.

[TYPOGRAPHY & VISUAL POLISH]
- Korean font: bold heavy sans-serif (Noto Sans KR Bold / Pretendard ExtraBold style).
  Avoid thin serifs, scripts, decorative fonts. Tight letter spacing (≈ -0.02em).
- Text color contrast:
  · light bg → solid black or deep brand color + 6-8px white halo
  · dark bg  → solid white + 8-10px black outline + soft drop shadow
  · saturated bg → white fill + thick black outline + subtle colored halo
- Text container: clean overlay with outline+shadow by default; rectangular sticker badge or
  angled banner only when it adds editorial polish. No random shapes/gradients on text.
- Visual hierarchy (size + contrast must follow): subject > headline > sub-tag > logo > background.

[ANALYTICS-DRIVEN CRITERIA — measurable for CTR re-analysis]
- Headline character height ≥ 12% of frame height (so Vision analyzer classifies text_size_level=large)
- Deep-shadow area < 40% of frame (brightness benchmark)
- Saturation visibly high (analyzer must classify "vivid/saturated", not "muted")
- Text/logo colors must NOT clash with the domain palette ({domain})
- Honor category-typical success patterns from the original analysis (person count, expression, framing)
...
{{
  ...
  "improvements": [
    ...
    #   일부 vision 모델에서 불안정 → 제거하고 프롬프트 레벨에서 JSON 강제
...
# ──────────────────────────────────────────────
# 5a. 이미지 생성 — Vertex AI + google-genai SDK (텍스트 → 새 썸네일)
# 모델: gemini-3-pro-image-preview (Pro Image, global endpoint)
# ──────────────────────────────────────────────
def gemini_gen_image(
    ...
    """
    Vertex AI + google-genai SDK로 텍스트 프롬프트만 받아 새 썸네일 이미지 생성.
    - gemini-3 계열: 자동으로 location='global'
    - gemini-2.5-flash-image: regional endpoint OK
    반환: PNG bytes
    """
    from google import genai
    from google.genai import types

    # 썸네일 특화 스타일 강제 — 한국 유튜브 썸네일 분석 베스트 프랙티스 반영
    enhanced = (
        f"{prompt}\n\n"
        "[Korean YouTube Thumbnail Style — STRICT]\n"
        "- VIVID, highly saturated colors with HIGH CONTRAST. BRIGHT well-lit scene.\n"
        "- Main subject fills 60-80% of frame in tight close-up. Subject must POP.\n"
        "- Simple, uncluttered background that doesn't compete with the subject.\n"
        "- Dynamic and energetic feel — NOT moody, NOT cinematic, NOT dark.\n"
        "- Wide horizontal 16:9 landscape ratio.\n\n"
        "[Text & Logo Policy]\n"
        "- IF the prompt above specifies Korean text, headlines, or brand logos: render them clearly.\n"
        "  * Bold sans-serif Korean characters, heavy weight\n"
        "  * Large enough to read on a small mobile thumbnail (text occupies ~25-35% of frame width)\n"
        "  * Thick outline or strong drop shadow so it stays legible on any background\n"
        "  * Maximum one main headline (2-5 Korean characters) + optional sub-tag (1-3 characters)\n"
        "  * Brand logo (if mentioned) small in a corner — lower-left or lower-right\n"
        "- IF the prompt does NOT specify text: leave one side (~30%) as clean empty space for later text overlay, "
        "and do NOT invent random text or letters.\n\n"
        "[Face Protection — NON-NEGOTIABLE]\n"
        "- Text and logos NEVER overlap or cover any person's face. Eyes, nose, mouth, and chin "
        "must remain fully visible.\n"
        "- Place text on the empty side opposite the face. Logo strictly in a lower corner away from the face.\n"
        "- If avoiding face overlap requires smaller text, shrink the text — do NOT cross the face."
    )
    ...
    for part in response.parts:
        ...
        if data:
            ...
            pil_img = _PilImage.open(BytesIO(data))
            out = BytesIO()
            pil_img.save(out, format="PNG")
            return out.getvalue()
...
# Step 2: 분석 결과 + 수정 지시 → 한국어→영어 번역
# Step 3: 참고 이미지 + 영어 final_prompt → gemini-3-pro-image-preview
# ──────────────────────────────────────────────
...
Be specific and concise. English only. Max 200 words."""
    ...
    img_part = types.Part.from_bytes(data=img_raw, mime_type="image/jpeg")
    desc_resp = client.models.generate_content(
        model=GEMINI_VISION_MODEL,
        contents=[img_part, desc_prompt],
        config=types.GenerateContentConfig(
            temperature=IMAGE_GEN_CONFIG["vision_temperature"],
            max_output_tokens=IMAGE_GEN_CONFIG["style_desc_max_tokens"],
        ),
    )
    style_desc = (desc_resp.text or "").strip()

    # ── Step 2: 수정 지시(한국어) → 영어 번역 후 최종 프롬프트 구성 ──
    # 한국어를 그대로 image gen 모델에 넣으면 이미지 안에 깨진 글자가 렌더링될 수 있음
    if edit_prompt.strip():
        _translate_prompt = (
            "Translate the following Korean YouTube thumbnail improvement guide "
            "into clear English for an image generation AI. "
            "Preserve the section structure (e.g. '[목표]' → '[Goal]', '[기존 썸네일 현황]' → '[Current State]', "
            "'[유지할 강점]' → '[Keep]', '[개선 방향]' → '[Improvements]', "
            "'[텍스트·로고 규칙 — 중요]' → '[Text & Logo Rules — CRITICAL]', "
            "'[제약 조건]' → '[Constraints]') "
            "and bullet points. Keep numbered items numbered. "
            "Return ONLY the English translation, no extra explanation, no markdown fences.\n\n"
            f"Korean:\n{edit_prompt.strip()}"
        )
        _tr_resp = client.models.generate_content(
            model=GEMINI_PROMPT_MODEL,
            contents=[_translate_prompt],
            config=types.GenerateContentConfig(
                temperature=IMAGE_GEN_CONFIG["translate_temperature"],
                max_output_tokens=IMAGE_GEN_CONFIG["translate_max_tokens"],
            ),
        )
        edit_en = (_tr_resp.text or "").strip()
    else:
        edit_en = "improve overall visual quality"

    _style_trim = style_desc[:IMAGE_GEN_CONFIG['style_desc_trim']]
    _hints_block = f"SPECIFIC HINTS:\n{analysis_hints}\n\n" if analysis_hints else ""

    final_prompt = (
        f"Task: Recreate this Korean YouTube thumbnail with improvements for {domain_ctx}.\n"
        f"The output will be re-scored by a Vision CTR analyzer — make every change measurable.\n\n"
        f"REFERENCE IMAGE PROVIDED: Use the attached thumbnail as the visual base. "
        f"Preserve its composition, framing, and brand identity unless explicitly changed below.\n\n"
        f"ORIGINAL THUMBNAIL ANALYSIS (Vision):\n{_style_trim}\n\n"
        f"REQUIRED IMPROVEMENTS:\n{edit_en}\n\n"
        f"{_hints_block}"
        f"TEXT & LOGO RULES (CRITICAL — preservation + polish):\n"
        f"- PRESERVE every text overlay, caption, and logo present in the reference. DO NOT remove them.\n"
        f"- Keep the original wording readable and brand logos identifiable.\n"
        f"- You MAY improve styling only: font (bold heavy Korean sans-serif — Noto Sans KR Bold / "
        f"Pretendard ExtraBold style, tight ≈ -0.02em letter spacing), weight, size, position, "
        f"color contrast, outline (8-10px black on dark / 6-8px white halo on light), drop shadow, "
        f"and integration with the background.\n"
        f"- Headline character height ≥ 12% of frame height so it stays readable on a 320-360px mobile thumbnail.\n"
        f"- Logo: keep brand primary color, place in lower-left or lower-right corner at 8-12% of frame width, "
        f"never overlapping the subject's face or main product.\n"
        f"- FACE PROTECTION (NON-NEGOTIABLE): text and logos MUST NEVER cover any person's face. "
        f"Eyes, eyebrows, nose, mouth, and chin must stay fully visible. "
        f"Place text on the clean side opposite the face. If the face fills the center, push text "
        f"to the bottom strip (≤20% frame height) under the chin. Shrink text before ever crossing the face.\n\n"
        f"TYPOGRAPHY & VISUAL POLISH:\n"
        f"- Text container: clean overlay with outline+shadow by default; rectangular sticker badge or "
        f"angled banner only if it adds editorial polish. No random shapes/gradients on text.\n"
        f"- Text color picks: light bg → solid black + white halo; dark bg → white + black outline + shadow; "
        f"saturated bg → white fill + thick black outline + subtle colored halo.\n"
        f"- Visual hierarchy (size + contrast must follow this order): "
        f"subject (face/product) > headline > sub-tag > logo > background.\n\n"
        f"ANALYTICS-DRIVEN CRITERIA (the Vision re-scorer will check these):\n"
        f"- Subject coverage 50-70% of frame (close-up tier)\n"
        f"- Deep-shadow area < 40% of frame (brightness benchmark)\n"
        f"- Saturation visibly vivid (classifier should NOT label it 'muted')\n"
        f"- Text/logo colors do NOT clash with the domain palette ({domain})\n"
        f"- Honor category-typical success patterns from the analysis (person count, expression, framing)\n\n"
        f"CONSTRAINTS:\n"
        f"- 16:9 horizontal aspect ratio\n"
        f"- High quality professional photography\n"
        f"- Korean brand thumbnail aesthetic, strong visual hook\n"
    )

    # ── Step 3: gemini-3-pro-image-preview로 참고 이미지 기반 생성 ──
    png_bytes = gemini_image_generate_with_reference(
        prompt=final_prompt,
        ref_image_bytes=img_raw,
            ...
            if cached and cached.get("video_id") == sel["id"]:
                ...
                with _btn1:
                    if st.button("🎨 개선 썸네일 제작하기",
                        ...
                        st.switch_page("thunbnail_generation_v1.py")
                with _btn2:
                    if st.button("💾 분석 결과 저장",
                        ...
                        _report_html = (
                            ...
                            f'<div style="font-size:11px;color:#9ca3af;margin-bottom:12px">{sel["title"][:60]}<br>'
