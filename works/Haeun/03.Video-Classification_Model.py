...
class VideoClassification(BaseModel):
    ...
    content_type: Literal[
        ...
        "브이로그",      # 특정 행사나 이벤트 없이 일상이나 현장을 자연스럽게 담은 영상
        ...
        "웹예능",        # 예능 형식으로 재미있게 만든 영상
        "이벤트/행사",   # 제품 런칭, 기자간담회, 팝업스토어 등 특정 행사 현장을 담은 영상 (일상 기록인 브이로그와 다름)
        "인터뷰",        # 임직원, 전문가, 고객 등 특정 인물과 대화하는 형식의 영상 (일상 기록인 브이로그와 다름)
        "애니메이션",    # 실사 촬영 없이 모션그래픽, 2D/3D 애니메이션으로 만든 영상

        # IT 특화 포맷
        "기술설명",      # 개발 개념, 알고리즘 등 따라하기보다 이해를 목적으로 한 순수 기술 정보 영상 (단계별 따라하기는 튜토리얼)
    ...
    cta_type: Literal[
        "구매유도",    # 영상을 보고 제품을 사도록 유도하는 경우
        "구독유도",    # 채널 구독이나 알림 설정을 요청하는 경우
        "이벤트참여",  # 댓글, 응모, 챌린지 등 이벤트에 참여하도록 유도하는 경우
        "정보탐색",    # 공식 홈페이지, 상세페이지 링크를 제공하는 경우
        "앱다운로드",  # 앱 스토어 링크로 앱 설치를 유도하는 경우 (IT 기업에서 자주 나옴)
        "방문유도",    # 오프라인 매장, 팝업스토어 등 현장 방문을 유도하는 경우 (F&B에서 자주 나옴)
        "없음",        # 시청자에게 특별히 뭔가를 하라고 요청하지 않는 경우
        "기타",        # 위 유형 중 어디에도 명확히 해당하지 않는 경우에만 선택
    ] = Field(description="이 영상이 시청자에게 무엇을 하도록 유도하는지 고른다.")

    is_series: bool = Field(
        description="EP, 1화, #1, 시즌 등의 표현이 제목에 있으면 True, 없으면 False"
    )

    is_collaboration: bool = Field(
        description="다른 브랜드나 크리에이터와 함께 만든 영상이면 True, 혼자 만든 영상이면 False"
    )
...
1. domain
   ...
   - 카테고리 ID 28(Science & Technology)이면 IT를 우선 고려해
   - 카테고리 ID 26(Howto & Style), 24(Entertainment)이면 F&B를 우선 고려해
...
2. content_type
   ...
   - 인터뷰는 특정 인물이 질문에 답하거나 경험·의견을 말하는 형식이 명확할 때만 선택해
     (예: "1등 바리스타의 원픽", "임직원이 말하는 회사 생활")
...
4. cta_type
   ...
   - 설명란이 비어있거나 단순 소개만 있으면 없음
   - 기타는 위 유형 중 어디에도 명확히 해당하지 않을 때만 선택해

5. is_series
   - 제목에 EP, 1화, 2화, #1, 시즌, Part 등의 표현이 있으면 True
   - 제목에 Track, Session, Day, Vol 등 순서를 나타내는 표현이 있으면 True
   - 없으면 False

6. is_collaboration
   - 제목이나 설명에 ft., feat., with, X(크로스), 콜라보 등의 표현이 있으면 True
   - 단, 자사 제품끼리의 콜라보(예: 갤럭시 X 버즈)는 False
   - 없으면 False
