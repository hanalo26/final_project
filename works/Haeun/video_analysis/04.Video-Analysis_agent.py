...
# ========================================
# TODO:OpenCV를 사용한 정량 분석 코드 작성
def analyze_quantitative(video_path, video_id):
    pass
...
async def analyze_one(
    ...
    - 전체 작업 순서
      (1) yt-dlp 라이브러리를 사용해 유튜브 숏츠 영상 다운로드 (URL 활용) --> 라이브러리 확인중.
      (2) OpenCV 라이브러리를 사용해 정량 분석 수행
        - Gemini 정성분석 결과 비교를 위해 허깅페이스에 공개된 무료 모델 활용 예정
    ...
    async with sem:
        ...
        video_path = download_video(row['url'], video_id, args.video_dir)
        ...
        # TODO:프롬프트 변경하기 (영상 파일을 Gemini에게 입력하도록 할 예정)
        prompt = None
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                combined['channel_name'] = row['channel_name']
