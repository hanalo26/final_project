"""
선정된 기업들의 유튜브 채널 수집 agent로 주요 업데이트 사항은 다음과 같습니다.

- request.get()에서 요청 제한 시간 추가
    - timeout=10 파라미터 추가

- 시스템 프롬프트 업데이트
    - 2개의 도구를 활용해 채널 ID를 수집하고 있어, 명시적으로 중복된 채널 ID는 제거하도록 추가
    
- F&B, IT 기업 리스트를 기반으로 유튜브 채널을 수집하는 로직이 완전히 동일하므로 `argparse` 라이브러리를
사용하여 터미널에서 실행할 때 파일 경로를 변수처럼 전달하도록 함
    -> 같은 로직을 사용하는 경우, 데이터 파일의 경로를 파일 실행할 때 추가하면 재활용 가능

- argparse 관련 코드 주석 추가
    - 터미널 실행 시 파일 경로를 변수처럼 전달받는 역할 명시
    - args.csv_path, Path().stem, CHECKPOINT_FILE 동적 생성 로직 설명 추가

- 파이썬 파일 실행하는 방법 - 예시
    (1) uv run (파이썬 코드 파일의 상대경로) ./data/식품기업리스트.csv
    (2) 동시 실행 수, 대기 시간을 바꾸고 싶다면 다음과 같이 실행 코드 작성
        uv run (파이썬 코드 파일의 상대경로) ./data/IT기업.csv --concurrent 5 --delay 5
...
import argparse # 터미널에서 코드를 실행할 때, 변수처럼 파일 경로를 전달할 수 있도록 하는 라이브러리
...
2단계 - 중복 채널 ID 제거
  - 1단계에서 수집한 채널 후보 중 동일한 값은 하나만 남겨둘 것
  - 같은 채널이라고 확실하게 판단되면 하나의 값만 남기고, 애매하면 3단계에서 검증하고
  중복 여부를 판별 후 중복값이라면 제거할 것

3단계 - 검증
  - 1,2단계를 통과한 채널 후보들의 모든 핸들과 채널 ID를 verify_youtube_channel로 검증
...
async def main():   
    ...
    df = pd.read_csv(args.csv_path, encoding="cp949")
    ...
    # Agent 호출
    all_results, stats, total_tokens = await run_agent(company_names)
