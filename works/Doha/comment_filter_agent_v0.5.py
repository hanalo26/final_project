"""
유튜브 댓글 스팸/홍보성 필터링 Agent + 이벤트 분류 + CTA 분석 (comment_filter_agent_v0.5.py)
...
[✅ 주요 개선사항 - v0.5]
────────────────────────────────────────────
1. is_event_score 필드 추가 (이벤트 참여 신호 점수)
2. is_event 필드를 점수 기반으로 변경
3. 스키마에 이벤트 분류 기준 상세 기록
4. 프롬프트에 이벤트 정의 반복 추가
5. 텍스트 기반 판별로 변경 ("우리 채널" 제외)
"""
...
def save_checkpoint(results: list):
    """결과를 JSON으로 체크포인트에 저장합니다."""
    with open(CHECKPOINT_FILE, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)


# ========================================
# 8. Tool: 댓글 컨텍스트 조회 (로컬 CSV에서)
# ========================================
def get_comment_context(comment_id: str) -> dict:
    """
    로컬 COMMENT_STORE에서 특정 댓글의 컨텍스트를 조회합니다.
...
# ========================================
SYSTEM_PROMPT = """
당신은 유튜브 댓글 필터링 전문 AI 에이전트입니다.
다음 기준에 따라 댓글을 정확하게 분류하세요:

[분류 기준]
1. Normal (정상 댓글): spam_score 0~4점
2. Spam (스팸): spam_score 5~7점 - 홍보성, 도배, 링크, 무관한 내용
3. Inappropriate (부적절): spam_score 8~10점 - 음란성, 성적 표현
